...
class Tools(AppBase):
    __version__ = "1.2.0"
    app_name = (
        "Oneflow Tools"  # this needs to match "name" in api.yaml for WALKOFF to work
    )
    ...
    # This is an SMS function of Oneflow
    def send_sms_Oneflow(self, apikey, phone_numbers, body):
        ...
        url = "https://Oneflowr.io/api/v1/functions/sendsms"
        headers = {"Authorization": "Bearer %s" % apikey}
        return requests.post(url, headers=headers, json=data).text

    # This is an email function of Oneflow
    def send_email_Oneflow(self, apikey, recipients, subject, body, attachments=""):
        ...
        url = "https://Oneflowr.io/api/v1/functions/sendmail"
        headers = {"Authorization": "Bearer %s" % apikey}
        return requests.post(url, headers=headers, json=data).text
    ...
    def execute_bash(self, code, Oneflow_input):
    ...
    def merge_lists(self, list_one, list_two, set_field="", sort_key_list_one="", sort_key_list_two=""):
        ...
        try:
            ...
            return {
                "success": False,
                "reason": "An error occurred while merging the lists. PS: List one can NOT be a list of integers. If this persists, contact us at support@Oneflowr.io",
                "exception": f"{e}",
            }
